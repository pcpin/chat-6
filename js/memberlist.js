    ...
    for (var i in members) {
      ...
      if (userlistAvatar) {
        if (members[i].AvatarBID>0) {
          urec=urec.split('[AVATAR_THUMB]').join('<img style="cursor:pointer" onclick="showUserProfile('+i+'); return false;" src="'+htmlspecialchars(formlink)+'?b_x='+htmlspecialchars(userlistAvatarHeight)+'&amp;b_y='+htmlspecialchars(userlistAvatarWidth)+'&amp;b_id='+htmlspecialchars(members[i].AvatarBID)+'&amp;s_id='+htmlspecialchars(s_id)+'" alt="'+htmlspecialchars(getLng('avatar'))+'" title="'+htmlspecialchars(getLng('avatar'))+'" border="0" />');
        } else {
