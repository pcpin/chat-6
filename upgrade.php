    ...
    if ($__pcpin_upgrade['db_version']<$__pcpin_upgrade['file_version']) {
      ...
      switch ($__pcpin_upgrade['db_version']) {
        ...
        case 6.11:
          ...
          $__pcpin_upgrade['session']->_db_query("CREATE TABLE `".PCPIN_DB_PREFIX."userdata_field` ( `id` int(10) unsigned NOT NULL auto_increment, `name` varchar(255) NOT NULL default '', `default_value` text NOT NULL, `custom` enum('y','n') default 'y', `type` enum('string','text','url','email','choice','multichoice') NOT NULL default 'text', `visibility` enum('public','registered','moderator','admin') NOT NULL default 'public', `writeable` enum('user','admin') NOT NULL default 'user', `order` int(10) unsigned NOT NULL, `disabled` enum('n','y') default 'n', PRIMARY KEY  (`id`), KEY `name` (`name`), KEY `visibility` (`visibility`), KEY `order` (`order`), KEY `disabled` (`disabled`) ) DEFAULT CHARSET=utf8 TYPE=MyISAM PACK_KEYS=0");
